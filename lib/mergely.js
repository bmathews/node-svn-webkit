...
jQuery.extend(Mgly.mergely.prototype, {
	...
	init: function(el, options) {
        ...
        CodeMirror.modeURL = options.cmsettings.modeURL;
	...
	bind: function() {
        ...
        CodeMirror.autoLoadMode(this.editor[this.id + '-lhs'], this.lhs_cmsettings.mode);
        CodeMirror.autoLoadMode(this.editor[this.id + '-rhs'], this.rhs_cmsettings.mode);
	...
	_diff: function(editor_name1, editor_name2) {
        ...
        if (this.changes.length && !this.firstScroll) {
            jQuery(this.editor[editor_name1].getScrollerElement()).scrollTop(this.changes[0]['lhs-y-start'] - 25);
	...
	_draw_diff: function(editor_name1, editor_name2, changes) {
		...
		for (var i = 0; i < changes.length; ++i) {
			...
			rectHeight = rhs_y_end - rhs_y_start;
